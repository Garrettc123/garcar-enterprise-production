"""FastAPI control surface for the complete 482-agent super-agent fabric."""
...
def activate_all():
    return {
        "status": "all_482_agents_activated",
        **_engine.inventory(),
        "execution_policy": {
            "economic_objectives": "enabled",
            "autonomous_spend_ceiling_cents": 2500,
            "external_side_effects": "explicit_tool_only",
            "high_risk_actions": "human_approval_required",
...
async def dispatch(name: str, request: DispatchRequest):
    try:
        return await _engine.dispatch(
            name,
            request.payload,
            request.dry_run,
            request.requested_spend_cents,
...
async def run_all(request: DispatchRequest):
    results = []
    for agent in AGENT_CATALOG:
        results.append(await _engine.dispatch(
            agent["name"], request.payload, request.dry_run,
            request.requested_spend_cents, request.human_approval,
        ))
